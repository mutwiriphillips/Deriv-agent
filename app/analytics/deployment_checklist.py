...
def generate_deployment_checklist(
    ...
    report.items.append(_not_automated(
        ...
        "no persistent lifetime forward-test trade log exists yet — SessionStats resets daily "
        "(app/monitoring/session_stats.py) and daily_state is keyed per calendar day "
        "(app/monitoring/state_persistence.py). Verify this manually from Deriv's own account "
        "statement until a lifetime log is built.",
    ))
    ...
    report.items.append(_not_automated(
        ...
        "no uptime or error-rate log exists yet — app/storage/schema.sql has a system_events table "
        "but nothing currently writes to it. Check Render's logs manually for repeated disconnects/errors.",
    ))
    ...
    report.items.append(_not_automated(
        ...
        "OrderManager measures latency per-trade (app/execution/order_manager.py) but nothing persists "
        "a history of it across trades yet to compute a percentile against. Check dashboard LATENCY "
        "readings manually across several trades.",
    ))
