...
def get_lifetime_stats(db_path: str, symbol: str | None = None) -> LifetimeStats:
    conn = sqlite3.connect(db_path)
    try:
        if symbol is not None:
            rows = conn.execute(
                "SELECT result, profit_loss, total_execution_latency_ms FROM trade_log WHERE symbol=?", (symbol,)
            ).fetchall()
        else:
            rows = conn.execute("SELECT result, profit_loss, total_execution_latency_ms FROM trade_log").fetchall()
    finally:
        conn.close()
